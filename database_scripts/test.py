...
def main(argv):
    ...
    taskID = d["data"]["_id"]
    assert(response.status == 201)
    params = urllib.parse.urlencode({'name': "user6", 'email': "user6@gmail", "pendingTasks": taskID})
    conn.request("POST", "/api/users", params, headers)
    response = conn.getresponse()
    data = response.read()
    d = json.loads(data)
    ...
    assert(response.status == 200)
    assert(d["data"]["assignedUser"] == userID)
    params = urllib.parse.urlencode({'name': "user7", 'email': "user7@gmail", "pendingTasks": taskID})
    conn.request("POST", "/api/users", params, headers)
    response = conn.getresponse()
    data = response.read()
    d = json.loads(data)
    ...
    taskID = d["data"]["_id"]
    assert(response.status == 201)
    params = urllib.parse.urlencode({'name': "user8", 'email': "user8@gmail", "pendingTasks": taskID})
    conn.request("POST", "/api/users", params, headers)
    response = conn.getresponse()
    data = response.read()
    d = json.loads(data)
    ...
    assert(d["data"]["assignedUser"] == userID)
    params = urllib.parse.urlencode({'name': "user9", 'email': "user9@gmail", "pendingTasks": taskID})
    conn.request("POST", "/api/users", params, headers)
    response = conn.getresponse()
    data = response.read()
    d = json.loads(data)
